						...
						$.ajax({
									...
									success : function(data) {
										for (var i = 0; i < data.length; i++) {
											$('#bookTable').append('<tr id='+data[i].id+'><td data-target=name>'+ data[i].title+'</td><td data-target=publications>'+data[i].author.authorName+ '</td><td> <a class="edit" data-id='+data[i].id+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete" data-id='+data[i].id+' title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a></td>');
										}
									}
								});
						$.ajax({
							...
							success : function(data) {
								for (var i = 0; i < data.length; i++) {
									$('#country').append('<option value="'+data[i].authorName+'" selected="">'+data[i].authorName+'</option>');
								}
							}
						});
						...
						$('#addBtn').on('click', function(){
							...
							$.ajax({
								...
								success:function(data){
									...
									$('#bookTable').append('<tr id='+data.id+'><td data-target=name>'+ data.title+'</td><td data-target=publications>'+data.author.authorName+ '</td><td> <a class="edit" data-id='+data.id+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete" data-id='+data.id+' title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a></td>');
					...
					<div class="row">
						...
						<div class="col-sm-4">
							...
							<button style="width: 100px" type="submit"
								class="btn btn-info add-new" data-toggle="modal"><a href="home.jsp" style="color: white;">Back</a></button>
						</div>
					</div>
				</div>
		...
		<div class="modal-content">
			...
			<form method="post" id="author_form">
				<div class="modal-body">
					...
					<div class="form-group">
						<label for="recipient-name" class="col-form-label" data-toggle="tooltip" data-placement="top" title="No of publications">Author Name</label> 
						 <div>
